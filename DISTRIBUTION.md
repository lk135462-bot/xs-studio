...
| **精簡版** | 同上；但要走 Claude 訂閱得自己先裝 Claude Code | 不會 | **27 MB zip** | 可以發 |
| 輕量版 | 先裝 Python，再解壓縮雙擊 | 不會 | 0.4 MB zip | 可以發，適合工程背景的人 |
| PyInstaller 版 | 解壓縮、雙擊 | **會**（SAC 封鎖／SmartScreen 警告） | 324 MB | 需簽章憑證才發得出去 |

**為什麼多一個精簡版**：內附的 Claude Code 佔掉九成體積，讓壓縮檔衝破 100 MB——
而 **Google Drive 只掃 100 MB 以下的檔案**，超過就對下載者顯示
「Google 無法掃描這個檔案是否含有病毒」（查證於 2026-08-21）。
那句話對非技術使用者是勸退級的。取捨：完整版讓訂閱者零安裝上手但下載時看到警告；
精簡版下載乾淨，代價是訂閱者要自己裝一次 Claude Code。
走 API Key／本地模型的人兩版都一樣。

**啟動點不能是 .bat**（這條踩過）：下載的檔案帶 Mark of the Web，
而檔案總管解壓縮會把標記傳給每個檔案。實測帶 MOTW 的 `.bat`／`.cmd`／`.lnk`
在 SAC 下**一律被封鎖**，只有已簽章的 exe 照常執行——所以啟動器是
`python.exe` 的副本改名，搭配 `._pth` 與 `sitecustomize.py` 做到雙擊即開。
